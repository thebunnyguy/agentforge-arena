...
class _Parser:
    ...
    def parse_factor(self):
        ...
        #
        # BUG: unary sign is only recognized when it is the very first token
        # of the *entire* expression (self.pos == 0). This is a common
        # incomplete fix for "unary minus crashes": it makes a leading
        # "-3+2" work, but misses every other position a unary sign can
        # legally appear (right after another operator, or right after an
        # opening parenthesis), where parse_factor falls through to
        # _parse_number and raises on the bare '-' or '+' token.
        tok = self._peek()
        if tok in ("+", "-") and self.pos == 0:
            self._next()
            value = self.parse_factor()
