...
**Timestamp**: 2026-09-17T21:15:24.039781+00:00  
**Duration**: 75522 ms
...
| `hidden_import_closure.gate7` | isolation | PASS | info | At most one distinct local file (['cachekit/__init__.py']) is reachable by a diff without failing the scope gate and imported by the hidden/regression suites — unambiguously the code under test, not a separate oracle-helper module. |
...
| `mutation.generic_ast_mutants` | mutation | PASS | info | All 14 relevant mutant(s) were killed by the hidden suite (kill_rate=1.00 over 17 generated, 0 unsupported, 0 declared-equivalent, 3 intercepted by the regression/scope gate before reaching the hidden suite). |
...
- Intercepted by regression/scope gate (never reached the hidden suite): 3
- Killed by the hidden suite: 14
- **Survived: 0**
